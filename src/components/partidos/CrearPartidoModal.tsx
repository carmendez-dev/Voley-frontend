...
import type { CrearPartidoDTO } from '../../types/partido.types';
...
const CrearPartidoModal: React.FC<CrearPartidoModalProps> = ({ onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (selectedTorneoId) {
      ...
      // Reset de selecciones dependientes
  ...
  useEffect(() => {
    if (selectedTorneoId && selectedCategoriaId) {
      cargarInscripciones(selectedTorneoId, selectedCategoriaId);
      // Reset de selección de inscripción
      setSelectedInscripcionId(null);
      setInscripciones([]);
    }
  }, [selectedCategoriaId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const partidoData: CrearPartidoDTO = {
        idInscripcionLocal: selectedInscripcionId!,
        idEquipoVisitante: selectedEquipoVisitanteId!,
        fecha: fecha,
        ubicacion: ubicacion.trim()
      };

      await partidosService.crearPartido(partidoData);
      onSuccess();
    } catch (err) {
      const apiError = handleApiError(err);
      setError(apiError.message);
      ...
      >
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center space-x-3">
            ...
            <h3 className="text-lg font-semibold text-gray-900">Crear Nuevo Partido</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {/* Sección: Selección de Equipo Local */}
          ...
          {/* Info sobre resultado inicial */}
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <p className="text-sm text-blue-700">
              <strong>Nota:</strong> El partido se creará con resultado "Pendiente" por defecto.
            </p>
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <button
              ...
              disabled={loading || !selectedInscripcionId || !selectedEquipoVisitanteId || !fecha || !ubicacion.trim()}
            >
